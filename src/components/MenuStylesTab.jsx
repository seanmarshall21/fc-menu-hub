...
/**
 * Merge an event-style override on top of the series base.
 * Both inputs are already normalized (every role/gap key present).
 * The override wins per-role and per-size; unmentioned keys fall through.
 */
function mergeSpec(base, override) {
  const out = { ...base }
  for (const r of ROLES) out[r.key] = { ...base[r.key], ...(override?.[r.key] || {}) }
  out.gaps = {
    sm: { ...base.gaps.sm, ...(override?.gaps?.sm || {}) },
    md: { ...base.gaps.md, ...(override?.gaps?.md || {}) },
    lg: { ...base.gaps.lg, ...(override?.gaps?.lg || {}) },
  }
  if (override?.dietary_icons)            out.dietary_icons    = override.dietary_icons
  if (override?.dietary_icon_size != null) out.dietary_icon_size = override.dietary_icon_size
  if (override?.logo_max_height   != null) out.logo_max_height   = override.logo_max_height
  return out
}
...
export default function MenuStylesTab({ menu, event, series, canEdit, onSaved }) {
  // Effective "inherited" base = series defaults merged with event overrides.
  // The menu's override sits on top of this combined base.
  const seriesBase   = normalizeSeriesSpec(series?.style_spec)
  const eventSpec    = event?.style_spec || {}
  const menuSpec     = menu.style_spec || {}

  // Build inherited spec — merge event on top of series for each piece.
  const inheritedSpec = mergeSpec(seriesBase, eventSpec)
  const inheritedFonts =
    (Array.isArray(event?.fonts) && event.fonts.length ? event.fonts :
     Array.isArray(series?.fonts) && series.fonts.length ? series.fonts :
     [{ key: 'primary', family: '', source: 'system', url: '' }])
  const inheritedHeader = event?.header_logo_url ?? series?.header_logo_url ?? ''
  const inheritedFooter = event?.footer_url      ?? series?.footer_url      ?? ''

  // Override toggles — does the menu itself have something set?
  const [overrideFonts,      setOverrideFonts]      = useState(Array.isArray(menu.fonts))
  const [overrideHeader,     setOverrideHeader]     = useState(menu.header_logo_url != null)
  const [overrideFooter,     setOverrideFooter]     = useState(menu.footer_url != null)
  const [overrideIcons,      setOverrideIcons]      = useState(menuSpec.dietary_icons != null)
  const [overrideRole,       setOverrideRole]       = useState(() => {
    const o = {}
    for (const r of ROLES) o[r.key] = menuSpec[r.key] != null
    return o
  })
  const [overrideGap,        setOverrideGap]        = useState({
    sm: menuSpec.gaps?.sm != null,
    md: menuSpec.gaps?.md != null,
    lg: menuSpec.gaps?.lg != null,
  })
  const [overrideIconSize,   setOverrideIconSize]   = useState(menuSpec.dietary_icon_size != null)
  const [overrideLogoH,      setOverrideLogoH]      = useState(menuSpec.logo_max_height != null)

  // Override values (only meaningful when toggle on) — seed with inherited values
  const [fonts,         setFonts]         = useState(Array.isArray(menu.fonts) && menu.fonts.length ? menu.fonts : inheritedFonts)
  ...
  useEffect(() => {
    ...
    const ms = menu.style_spec || {}
    const ov = {}
    for (const r of ROLES) ov[r.key] = ms[r.key] != null
    setOverrideRole(ov)
    setOverrideGap({ sm: ms.gaps?.sm != null, md: ms.gaps?.md != null, lg: ms.gaps?.lg != null })
  ...
  async function uploadToSeriesAssets(file, subpath) {
    ...
    const path = `${series.id}/menu-${menu.id}-${subpath}-${Date.now()}.${ext}`
  ...
  async function handleSave() {
    ...
    // Assemble menu.style_spec containing only overridden keys
    const newSpec = {}
    ...
    const { error: err } = await supabase.from('menus').update(payload).eq('id', menu.id)
    setSaving(false)
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="card p-5 space-y-4">
        ...
        />
        {!overrideFonts ? (
          <InheritedPreview rows={inheritedFonts.map(f => `${f.key} · ${f.family || '(no family)'} · ${f.source}`)} />
        ) : (
      ...
      <section className="card p-5 space-y-3">
        ...
        {!overrideHeader ? (
          <InheritedAsset url={inheritedHeader} />
        ) : (
      ...
      <section className="card p-5 space-y-3">
        ...
        {!overrideFooter ? (
          <InheritedAsset url={inheritedFooter} />
        ) : (
      ...
      <section className="card p-5 space-y-3">
        ...
        {!overrideIcons ? (
          <InheritedPreview rows={['vegetarian', 'vegan', 'gf'].map(k => {
            const v = inheritedSpec.dietary_icons[k]
      ...
      <section className="card p-5 space-y-4">
        ...
        <div className="space-y-4">
          {ROLES.map(role => {
            ...
            const v = isOverride ? roleSpecs[role.key] : inheritedSpec[role.key]
            return (
      ...
      <section className="card p-5 space-y-4">
        ...
        <div className={`grid grid-cols-2 sm:grid-cols-3 gap-3 ${!overrideGap[activeSize] ? 'opacity-50 pointer-events-none' : ''}`}>
          {(() => {
            const v = overrideGap[activeSize] ? gapSpecs[activeSize] : inheritedSpec.gaps[activeSize]
            return (
              <>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-3 border-t border-surface-100">
          <OverridableNumber label="Dietary icon size" override={overrideIconSize} value={iconSize} onChange={setIconSize}
            seriesValue={inheritedSpec.dietary_icon_size} onToggleOverride={() => setOverrideIconSize(o => !o)} suffix="px" readOnly={readOnly} />
          <OverridableNumber label="Logo max height"   override={overrideLogoH}   value={logoH}    onChange={setLogoH}
            seriesValue={inheritedSpec.logo_max_height} onToggleOverride={() => setOverrideLogoH(o => !o)} suffix="px" readOnly={readOnly} />
        </div>
      </section>
