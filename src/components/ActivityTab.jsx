...
export default function ActivityTab({ scopeType, scopeId, open, onOpen }) {
  ...
  return createPortal(
    <button onClick={onOpen} title="Activity & feedback"
      className={`fixed right-0 top-[88px] z-[115] flex items-center justify-center rounded-l-xl shadow-lg py-2.5 pl-3 pr-2.5 transition-colors ${
        unread ? 'bg-brand-600 hover:bg-brand-700 text-white' : 'bg-surface-200 hover:bg-surface-300 text-ink-500'}`}>
